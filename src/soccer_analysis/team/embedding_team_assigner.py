...
Satisfies ``soccer_analysis.interfaces.TeamClassifier``. Crops each player
...
the way ``soccer_analysis.team.team_assigner.TeamAssigner`` does. General
...
``soccer_analysis.detection.onnx_tracker`` for where that limitation
...
from soccer_analysis.geometry.bbox import BBox
