...
same pattern as ``agon.detection.tracker``'s torch import, so
...
``agon.geometry.pitch_keypoint_calibrator``) prefer "no answer"
    ...
    def __init__(self, calibration: ClockCalibrationConfig):
        try:
            ...
            raise ImportError(
                ...
                "-- pip install 'agon[ocr]' for the Python wrapper, "
