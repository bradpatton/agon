...
Satisfies ``agon.interfaces.Detector``. This backend needs the
optional ``train`` extra (``pip install agon[train]``, which pulls
...
``agon.detection.onnx_tracker.OnnxDetector``, the default backend.

The ``ultralytics`` import is deliberately deferred to ``__init__`` (not
module level) so that importing ``agon.detection`` doesn't require
...
class UltralyticsDetector:
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "UltralyticsDetector needs the 'train' extra: pip install 'agon[train]'"
            ) from e
